...
- **FAIL-03 / FAIL-04 / FAIL-05 / FAIL-07** — Offline voice upload stays queued (no mid-flow error). Whisper failure retries the original `documentDirectory` recording on the same quote. GPT mapping failure keeps a flagged partial draft plus Add items. Crash/kill writes a local `resume_checkpoints` row and prompts **Resume where you left off**. Remaining **FAIL-*** (SMS, FCM, FAIL-01/02) are not done.
