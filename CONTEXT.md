...
- **SYNC-04** — Quotes/Catalog banner + header warning open a Sync issues screen of `dead_letter` queue items (plain-language entity/action + Retry; freeze / status-lock reasons). Live WatermelonDB observe; Retry resets to `pending` and kicks `processQueue` without rewriting stored totals or prices (PR #75).
...
- **Thin old-quote import (design §5, PR #77 empty/OCR-stub copy)** — Optional onboarding + Catalog **Import old quotes**. Paste priced lines (fixture-tested parser) → upsert `rate_card_entries` with `source=imported` via existing `POST /rate-card` / `POST /rate-card/import`. Empty paste is a calm skip-and-quote block; partial parse lists skipped lines and does not invent dollars. Pick up to 3 photos/screenshots as a hook; **image/PDF OCR is a stub** (honest paste hint, never blocks quoting). Does not invent prices, does not write catalog SKUs, does not replace P0-C skip.
- **Thin rate card list (design §10, PR #76 polish)** — Rate card stays a byproduct, not onboarding. `GET /rate-card` without `name` lists the contractor’s entries (`{ entries, limit, offset, total }`; `name`+`unit` is still exact lookup). Optional `q` or `search` filters that list by normalized-name substring; optional `unit` chip is list UX only — voice attach still uses exact `GET /rate-card?name&unit`. Catalog-adjacent **My rates** shows last price, unit, use count, and stored `source` (including `imported`), with a search field that calls list with `q` and a unit chip. Filtered miss is **No rates match** (no suggested rows). Tap edits price via existing `POST /rate-card` upsert (does not invent rows). Optional swipe delete is `DELETE /rate-card/:id`; the list refreshes without inventing a replacement. No embeddings/fuzzy search.
...
- **PR #73** — Docs-only CONTEXT sync through merged #71–#72. No product code.
- **PR #74** — Metro UAT checklist for first-win quoting ([docs/METRO-UAT-CHECKLIST.md](docs/METRO-UAT-CHECKLIST.md)). Docs only; physical UAT not signed off.
- **PR #75** — Sync issues dead-letter retry UX: plain-English freeze / status-lock reasons; Retry still `pending` + `processQueue`; never rewrites stored totals or prices.
- **PR #76** — My rates unit chip filter + empty-delete edges (**No rates match** on filter miss; confirmed delete refreshes without inventing rows or prices). Voice attach stays exact `name`+`unit`.
- **PR #77** — Paste-import empty and OCR-stub copy: calm empty paste; skipped lines listed; photos/PDFs still a paste stub; never invents prices.
...
- **Old-quote OCR** is not shipped: photos/PDFs can be picked (max 3) but extraction is a paste stub (honest copy; we will not guess prices from a picture). Empty paste is a calm skip-and-quote block; partial parse lists skipped lines and does not invent dollars. Working path is paste lines → `source=imported` rate-card upsert.
...
| `docs/METRO-UAT-CHECKLIST.md` | Founder Metro morning UAT (physical device, after 14:00): first-win quoting. Not Phase 6 SMS |
