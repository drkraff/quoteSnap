...
Re-check GitHub before treating anything else as landed. This briefing includes merged PRs through **#66** (P0 pricing tests; **#63** draft Import from photo stub). Overnight 2026-09-14/15 on `master`: P0 rate card / adhoc voice / skippable seed + hourly, thin SYNC-06, FAIL-03/04/05/07, private notes, price_source, option groups, client sentence, rooms, photos, PDF share, paste→rate card, markup compute, My rates list, mark-sent on share. **FAIL-01** (this map) and **FAIL-02** (mic Alert already in `voice-record.tsx`) are documented in [docs/WORKFLOW-failure-edge-cases.md](docs/WORKFLOW-failure-edge-cases.md). **Not** in that wave: Phase 6 SMS/Twilio/approval page, or GPT-4o Vision (`PHOTO-01`). Draft **Import from photo** is a stub on the existing photo-on-line path (blank price; filename / “Imported item”).
...
- **Client sentence (design §8, PR #52)** — Customer-facing assumption line on `quotes.client_sentence` (API `clientSentence`; max 2000). Voice may join extract `assumptions[]` when present; empty extract does not invent copy. Draft can edit it. `toCustomerQuotePayload` includes it; private notes stay out.
...
15. **Share-mark-sent is not SMS.** After a successful customer PDF/HTML share, eligible drafts (`draft_local` / `draft_queued` / `ai_failed`) may PUT `status: sent` and set `sent_at` so thin SYNC-06 freeze applies. Do **not** invent `customer_phone`. Already-sent share is a no-op. The in-app **Send** button still only queues `draft_queued` — no Twilio, no approval page (`SMS-01`…`SMS-10` remain Phase 6).
