...
Re-check GitHub before treating anything else as landed. This briefing includes merged PRs through **#59** (material cost × markup). **Thin My rates list** (design §10) is in this tree.
...
- `(app)` — quotes list (default authenticated entry after login/restore/onboarding), catalog, `my-rates`, `import-quotes`, `voice-record`, `draft/[id]`, `quote/[id]`. Draft and quote detail have **Share quote** (OS share sheet). No Home tab; **Log Out** is a nav-header action (A-19). `/(app)` / index redirects to Quotes.
